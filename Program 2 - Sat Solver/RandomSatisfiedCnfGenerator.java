...
/** This <tt>Generator</tt> generates random satisfiable CNF formulas
 * of a fixed size over a fixed number of symbols and with a fixed
 * number of literals per clause.  This class also includes a
 * <tt>main</tt> method for testing.
 */
public class RandomSatisfiedCnfGenerator implements Generator {
     ...
     */
    public RandomSatisfiedCnfGenerator(int num_clauses, int num_symbols, int num_literals) {
	random = new Random();
    ...
    public Cnf getNext() {
	Cnf cnf = new Cnf();
	Literal[] clause = new Literal[num_literals];
	for (int j = 0; j < num_literals; j++)
	    clause[j] = new Literal(0, false);
	...
	for (int i = 0; i < num_clauses; i++) {
	    do {
		for (int j = 0; j < num_literals; j++) {
		    clause[j].sign = random.nextBoolean();
		    clause[j].symbol = random.nextInt(num_symbols);
		}
	    } while (!Cnf.isClauseSat(clause, model));
	    cnf.addClause();
	    for (int j = 0; j < num_literals; j++) {
		cnf.addLiteral("X" + clause[j].symbol, clause[j].sign);
	    }
	}
	...
	return "Generates random satisfiable CNF formulas over " +
	    num_symbols + " symbols, with " + num_clauses +
	    " clauses and " + num_literals + " literals per clause.";
    }
	...
	Generator gen = new RandomSatisfiedCnfGenerator(num_clauses,
							num_symbols,
							num_literals);
